...
def build_sort_prompt(ingredient_list):
    ...
    return f"""
Sort the following ingredient list into the order they would typically appear while walking through a grocery store.
...
SECTION RULES:
- Group ingredients under section headers.
- Use ONLY these exact section names (all caps):
  PRODUCE, DAIRY, DRY GOODS, CANNED, BEVERAGES, SPICES, OILS, BAKERY, MISC
- Each section header MUST be formatted exactly as:
  === SECTION NAME ===
- Do NOT create empty sections.
- Do NOT place ingredients outside of a section.
- Each section header must appear BEFORE its ingredients.
- Do NOT repeat section headers.

CRITICAL FORMATTING RULES:
- The output must be a flat list of strings.
- Section headers must be included as items in the list.
- Do NOT include any extra text, notes, or formatting.
