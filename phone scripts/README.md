...
```powershell
curl.exe -sS --noproxy "*" --max-time 8 http://127.0.0.1:5059/ -o NUL -w "%{http_code}`n"
```
...
```powershell
curl.exe -sS --noproxy "*" --max-time 8 http://192.168.68.62:5059/ -o NUL -w "%{http_code}`n"
```
