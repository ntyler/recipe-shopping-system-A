...
Optional SMTP settings for account verification, password reset, two-factor recovery, and account deletion verification emails:
...
- Set `SHOPPING_APP_PASSWORD_RESET_BASE_URL` to the address users should open from password reset and two-factor recovery emails, such as your LAN, Tailscale, or public HTTPS URL. If unset, reset emails use the current request host.
...
- Disable and backup-code regeneration require an authenticator code or backup code.
- Lost authenticator recovery emails are tied to the account that requested the recovery link. Opening the link should not switch to another signed-in account.
- Account action pages for password reset, two-factor recovery, and account deletion should remain visible until the user completes the action or clicks cancel.
...
19. Confirm two-factor authentication is account-specific. A recovery link emailed to one user should disable only that user's two-factor settings.
20. Confirm account action pages for password reset, two-factor recovery, and account deletion do not collapse into a blank screen before the user completes or cancels the action.
