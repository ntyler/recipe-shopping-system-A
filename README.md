...
- Social/video recipe imports, including YouTube Shorts, use public title, caption, description, transcript text, and local parsing first. If text-only extraction cannot find ingredients and `OPENAI_API_KEY` is set, the app can fall back to audio transcription plus video thumbnail images to ask OpenAI for a recipe. The fallback is tracked as `social-video-audio-image-extraction` in AI Usage & Billing.
