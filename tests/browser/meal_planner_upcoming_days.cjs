...
(async () => {
    ...
    try {
        ...
        assert.deepEqual(await custom.locator('[data-schedule-action="date"][aria-pressed="true"]').evaluateAll(nodes=>nodes.map(node=>node.dataset.date)),['2026-09-25','2026-09-26','2026-09-27','2026-09-28']);
        assert.equal(await custom.locator('.is-yield-short').count(),0);
        assert.match(await row(0).locator('[data-meal-yield-remaining]').textContent(),/All servings from one full recipe are planned/);
        await custom.locator('.meal-schedule-calendar').scrollIntoViewIfNeeded();await screenshot('upcoming-days-applied-desktop.png');
