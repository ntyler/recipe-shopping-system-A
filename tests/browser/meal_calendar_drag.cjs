...
(async () => {
    ...
    try {
        ...
        const editor = i => dialog.locator('[data-meal-editor]').nth(i);
        ...
        const ready = () => page.waitForFunction(() => document.getElementById('mealPlannerDialog').mealPlanScheduleState.entries.every(e => !e.panel.ui.loading));
        await page.getByRole('button',{name:'Add Meals',exact:true}).click();await ready();
        await editor(0).locator('[name="recipe_url"]').selectOption('recipe://bread');
        ...
        await editor(1).locator('[name="recipe_url"]').selectOption('recipe://soup');
        await editor(1).getByRole('button',{name:'Select days',exact:true}).click();
        await drag(1,'2026-10-27','2026-11-02');
        assert.deepEqual(await selected(1),['2026-10-05','2026-10-27','2026-10-28','2026-10-29','2026-10-30','2026-10-31','2026-11-01','2026-11-02']);
        ...
        assert.match(await editor(0).locator('[data-meal-editor-error]').textContent(),/select a recipe/);
