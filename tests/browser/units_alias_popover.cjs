...
(async () => {
    ...
    try {
        ...
        const add = row.getByRole('button', {name: 'Add alias', exact: true});
        const suggest = row.getByRole('button', {name: 'Suggest aliases', exact: true});
        ...
        assert.equal(await add.getAttribute('title'), 'Add alias'); assert.equal(await suggest.getAttribute('title'), 'Suggest aliases');
        ...
        await add.focus(); const before = await scroll(); await add.press('Enter');
        ...
        await input.press('Escape'); assert(await focused(suggest));
        ...
        await suggest.click(); await form.getByText('Suggesting aliases…', {exact:true}).waitFor();
        ...
        await suggest.click(); await chips.getByRole('button', {name:'fresh suggestion',exact:true}).waitFor();
        ...
        await other.locator('[data-unit-row-alias="add"]').click();
        release();
        ...
        await suggest.click(); await form.getByText('Suggestions temporarily unavailable.', {exact:true}).waitFor();
        ...
        await page.setViewportSize({width:1440,height:900}); await add.click();
