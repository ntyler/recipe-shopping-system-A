...
(async () => {
    ...
    try {
        ...
        const custom=row.locator('[data-meal-editor-form]');
        assert.equal(await save.textContent(),'Save 4 Meals');assert.equal(posts.length,0);
        assert(await custom.isVisible());
        assert.deepEqual(await custom.locator('[data-schedule-action="date"][aria-pressed="true"]').evaluateAll(nodes=>nodes.map(node=>node.dataset.date)),['2026-09-26','2026-09-27','2026-09-28','2026-09-29']);
        await custom.locator('.meal-schedule-calendar').scrollIntoViewIfNeeded();await screenshot('people-days-applied-desktop.png');
