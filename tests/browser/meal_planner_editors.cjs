...
(async () => {
    ...
    try {
        ...
        assert.match(await dialog.locator('[data-meal-batch-help]').textContent(),/3 recipes · 3 meals · 24 servings/);
        ...
        assert.equal(await servings(custom(1)).inputValue(),'2.666666');
        ...
        await row(1).locator('[data-meal-editor-customize]').click();
        assert.equal(await servings(custom(1)).inputValue(),'3','Reset rejoins the current shared settings');
        await custom(1).getByRole('button',{name:'One day',exact:true}).click();
        ...
        assert.equal(plans['recipe://rice'].allocations.length,2);assert.equal(plans['recipe://rice'].allocations[0].planned_servings,4.5);
