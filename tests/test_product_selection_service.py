...
class ProductSelectionServiceTest(unittest.TestCase):
    ...
    def test_current_recipes_and_cookbooks_use_compact_home_summaries(self):
        ...
        self.assertIn("View Cookbook As", cookbook_template)
        self.assertIn("Recipe View", cookbook_template)
        self.assertIn("Cookbook Menu", cookbook_template)
        self.assertIn("data-cookbook-view-mode-select", cookbook_template)
        ...
        self.assertIn("function setAllCookbookRecipesCollapsed", script)
        self.assertIn("function returnToCookbookAfterRename", script)
        self.assertIn("form.dataset.cookbookId = cookbookId;", script)
        self.assertIn("returnToCookbookAfterRename(cookbookId)", script)
        self.assertIn("restoreScroll: false", script)
        self.assertIn('button.closest("[data-cookbook-card]")', script)
        self.assertIn("function applyCookbookRecipeSearch", script)
        self.assertIn("function applyCookbookViewMode", script)
        ...
        self.assertIn(".cookbook-recipe-log-view", css)
    ...
    def test_purge_unclassified_cookbook_recipe_urls_only_clears_unclassified(self):
        ...
        with TemporaryDirectory() as temp_dir, patch.object(
            ...
            purged_urls = cookbook_service.purge_unclassified_cookbook_recipe_urls("unclassified")

            self.assertEqual(purged_urls, ["https://example.com/chili", "https://example.com/soup"])
            data = cookbook_service.load_cookbooks()
            self.assertEqual([cookbook["id"] for cookbook in data["cookbooks"]], ["unclassified", "meal-prep"])
            unclassified = data["cookbooks"][0]
            meal_prep = data["cookbooks"][1]
    ...
    def test_screen_settings_section_stays_at_top_and_opens_from_account_menu(self):
        ...
        self.assertIn('<section id="screenSettingsCard" class="app-card screen-settings-card" hidden>', screen_template)
        self.assertNotIn("data-screen-settings-panel", screen_template)
        self.assertIn("closeScreenSettingsPanel()", screen_template)
        self.assertIn('data-screen-mode-button="phone"', screen_template)
        self.assertIn('id="screenPreviewFrame"', screen_template)
        self.assertIn("body.screen-settings-open.screen-preview-active #appContent", css)
        self.assertNotIn("body.screen-preview-active #appContent", css)
        ...
        self.assertIn("function closeScreenSettingsPanel()", script)
        self.assertIn('document.getElementById("screenSettingsCard")', script)
        self.assertIn('document.body.classList.add("screen-settings-open")', script)
        self.assertIn('document.body.classList.remove("screen-settings-open")', script)
    ...
    def test_store_options_toolbar_only_runs_nearest_stores(self):
        ...
        self.assertIn("Find Nearest Stores", store_template)
        ...
        self.assertIn('data-lazy-section="store-options"', index_template)
        self.assertNotIn("current_user.is_authenticated", index_template)
