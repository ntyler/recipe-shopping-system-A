...
const mealsPanel=node({querySelectorAll(selector){return calendarNodes.filter(item=>calendarMatches(item,selector));}});
const page=node({dataset:{mealWeek:'2026-09-21'},querySelector(selector){return selector==='#plannerMealsPanel'?mealsPanel:null;}});
function makeDialog() {
 const options=[
...
function cardMenu(overrides={}) {
 ...
 const firstAction=node(),actions=[firstAction];
 const menu=node({dataset:{triggerId:trigger.id},style:{},popoverOpen:false,actions,
...
assert.equal(items.otherBatch.dataset.recipeUrl,items.target.dataset.recipeUrl,'The same recipe may belong to a different batch');
assert(!items.otherBatch.classList.contains('is-removal-preview'));assert(!items.outside.classList.contains('is-removal-preview'));
menuEvent(card.menu,'pointerout',batch,card.firstAction);assert.deepEqual(highlighted(items),[],'Moving to a non-removal action clears the preview');
menuEvent(card.menu,'pointerover',batch);menuEvent(card.menu,'pointerout',batch,node());assert.deepEqual(highlighted(items),[]);
assert.equal(requests.length,0,'Hovering must not GET details or DELETE any scheduled record');assert.equal(activeDialog.open,false);
""")
...
menuEvent(card.menu,'focusout',batch,single);assert.deepEqual(highlighted(items),['target']);
menuEvent(card.menu,'focusin',card.firstAction);assert.deepEqual(highlighted(items),[]);
menuEvent(card.menu,'focusin',batch);menuEvent(card.menu,'focusout',batch,node());assert.deepEqual(highlighted(items),[]);
menuEvent(card.menu,'pointerover',batch,null,{pointerType:'touch'});assert.deepEqual(highlighted(items),[],'Touch has no hover preview');
batch.disabled=true;menuEvent(card.menu,'focusin',batch);assert.deepEqual(highlighted(items),[]);
assert.equal(requests.length,0);
""")
...
menuEvent(card.menu,'pointerover',batch);menuEvent(card.menu,'focusin',batch);
assert.deepEqual(highlighted(items),[]);assert.equal(requests.length,0);
""".replace("__STATE__", repr(hidden_state)))
...
let confirmed=false;
if(dismissal==='outside')ctx.document.dispatch('pointerdown',node());
if(dismissal==='escape')ctx.document.dispatch('keydown',node(),{key:'Escape',preventDefault(){}});
if(dismissal==='native')card.menu.hidePopover();
if(dismissal==='toggle')ctx.toggleMealPlannerCardMenu(card.trigger);
if(dismissal==='switch'){const other=cardMenu({menuId:'second-menu',mealId:'other',batchId:'another-batch'});ctx.toggleMealPlannerCardMenu(other.trigger);}
if(dismissal==='scroll')ctx.document.dispatch('scroll',page);
if(dismissal==='resize')ctx.window.dispatch('resize',ctx.window);
if(dismissal==='confirmation'){
...
for(const event of ['pointerover','pointerout','focusin','focusout'])assert.equal(card.menu.handlers[event].length,1,'Reopening does not bind duplicate preview listeners');
ctx.document.dispatch('pointerdown',node());assert.deepEqual(highlighted(items),[]);assert.equal(requests.length,0);
""")
...
requests=[];responseFactory=async(url,options)=>{
 assert.equal(url,'/api/meal-plan/batches/bulk');assert.equal(options.method,'POST');
 const body=JSON.parse(options.body).batches[0];
...
M.setHouseholdDefault(shared.draft,'lunch',3);M.setDates(shared.draft,['2026-10-06']);shared.render();
assert.equal(Number(third.panel.draft.householdDefaults.lunch),2.5);
assert.equal(third.panel.draft.notes,'Third only');
assert.match(second.root.querySelector('[data-meal-editor-summary]').textContent,/Uses shared plan.*3 servings/);
ctx.customizeMealPlannerRecipe(activeDialog,third);assert.equal(third.expanded,false,'Collapse preserves the override');
ctx.removeMealPlannerEditor(activeDialog,second);
assert.equal(state.entries[1],third);assert.equal(third.id,thirdId);
assert.equal(third.root.querySelector('[data-meal-editor-number]').textContent,'Recipe 2');
...
assert.equal(body.batches[0].allocations[0].date,'2026-10-06');
assert.equal(body.batches[1].allocations[0].date,'2026-10-09');assert.equal(body.batches[1].prep_notes,'Third only');
assert.equal(activeDialog.open,false);
""")
...
assert.equal(shared.draft.familyDefaults.child.dinner.servings,0.5);
assert.equal(activeDialog.querySelector('[data-meal-batch-save]').textContent,'Save 3 Meals');
ctx.resetMealPlannerRecipe(activeDialog,second);assert.equal(second.panel,null);assert.equal(second.expanded,false);
assert.equal(activeDialog.querySelector('[data-meal-batch-save]').textContent,'Save 4 Meals');
ctx.addMealPlannerEditor();assert.equal(activeDialog.querySelector('[data-meal-batch-save]').textContent,'Save 4 Meals');
requests=[];responseFactory=async()=>ok({batches:[],meals:[]});await ctx.saveMealPlannerBatch();
const body=JSON.parse(requests[0].options.body);assert.equal(body.batches.length,2);
assert.deepEqual(body.batches[0].allocations,body.batches[1].allocations);
assert.equal(body.batches[1].prep_notes,'Shared changes');
assert.equal(body.batches[1].allocations[0].member_portions[1].servings,0.5);
""")
...
assert.equal(JSON.parse(requests[0].options.body).batches.length,2,'Ignore untouched placeholder');
""")


def test_successful_bulk_save_cannot_be_retried_when_refresh_fails():
    run_dialog(r"""
await open();const panel=await choose('recipe://bread');requests=[];
responseFactory=async()=>ok({batches:[{id:'saved'}],meals:[]});
ctx.refreshRecipePreviewMeals=async()=>{throw new Error('Refresh failed');};
await ctx.saveMealPlannerBatch();
assert.equal(requests.length,1);assert.equal(activeDialog.mealPlanScheduleState.entries.length,0);
assert.equal(activeDialog.open,false);assert.equal(panel.ui.saved,true);
assert.match(activeDialog.querySelector('[data-meal-planner-refresh-status]').textContent,/Meals saved/);
...
const pending=ctx.saveMealPlannerBatch();await flush();
...
assert.equal(ctx.mealPlannerEditorContext(activeDialog,entry).ingredient_option_selections.butter,'unsalted');
...
assert.equal(ctx.mealPlannerEditorContext(activeDialog,activeDialog.mealPlanScheduleState.entries[0]).recipe_url,'recipe://soup');
