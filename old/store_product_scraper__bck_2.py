...
def update_home_store_on_existing_driver(
    ...
    try:
        ...
        # Aldi needs a dedicated path: click Change store, type the HOME address,
        # then select the matching Aldi card from the list.
        if store == "aldi":
            return click_aldi_change_store_flow(
                driver=driver,
                context=context,
                worker_id=worker_id,
                wait_seconds=wait_seconds,
