...
WORKER_PROFILE_DIR = SCRIPT_DIR / "chrome_worker_profiles"

DEFAULT_CHROME_USER_DATA_DIR = Path(r"C:\Users\Tyler\AppData\Local\Google\Chrome\User Data")
...
ADDRESS_FILE = SCRIPT_DIR / "address.json"
...
def load_address():
    if not ADDRESS_FILE.exists():
        raise FileNotFoundError(f"Missing {ADDRESS_FILE}")

    data = json.loads(ADDRESS_FILE.read_text(encoding="utf-8"))

    return (
        data.get("address_street", ""),
        data.get("address_appartment", ""),
...
address_street, address_appartment, address_city, address_state, address_zip = load_address()
...
def build_driver(
    *,
    ...
    if use_default_profile:
        options.add_argument(f"--user-data-dir={DEFAULT_CHROME_USER_DATA_DIR}")
        options.add_argument(f"--profile-directory={DEFAULT_CHROME_PROFILE_DIR}")
        print(f"[worker {worker_id}] ✅ Using DEFAULT Chrome profile")
    else:
        WORKER_PROFILE_DIR.mkdir(parents=True, exist_ok=True)
        profile_path = WORKER_PROFILE_DIR / f"worker_{worker_id}"
        profile_path.mkdir(parents=True, exist_ok=True)
        options.add_argument(f"--user-data-dir={profile_path}")
        print(f"[worker {worker_id}] ✅ Using worker Chrome profile: {profile_path}")
...
def sort_candidates_by_closest_address(candidates: list[dict[str, Any]]) -> list[dict[str, Any]]:
    enriched = []
    ...
    for item in candidates:
        ...
        copied["address_compare_score"] = address_similarity_score(copied.get("address"), full_address)
...
def find_best_embedded_link(parent, store_key: str, base_url: str) -> tuple[str | None, str | None]:
    ...
    best = sorted(links, key=lambda item: item["score"], reverse=True)[0]
...
- Compare every candidate address against FULL ADDRESS TO COMPARE_AGAINST.
...
def get_locator_search_value(store_key: str, stores: dict[str, Any]) -> tuple[str, str]:
    locator_search_by = stores.get(store_key, {}).get("locator_search_by") or "Zip Code"
...
##        return f"{locator_url}/?q={quote_plus(search_value)}"
    ...
    return f"{locator_url}{search_value}"
...
def search_locator(
    ...
    locator_url = stores.get(store_key, {}).get("locator_url") or DEFAULT_LOCATORS.get(store_key)
...
def run_store_worker(
    ...
    try:
        ...
        nearest = search_locator(
            driver=driver,
            store_key=store_key,
            stores=stores_snapshot,
            debug=debug,
            pause=pause,
            wait_seconds=wait_seconds,
        )

        return store_key, nearest
...
def main() -> int:
    ...
    parser.add_argument(
        "--headed",
        action="store_true",
        help="Show Chrome windows.",
    )

    parser.add_argument(
        "--headless",
        action="store_true",
        help="Run Chrome hidden.",
    )

    parser.add_argument(
        "--use-default-profile",
        action="store_true",
        help="Use your real default Chrome profile. Safe only with --workers 1.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Number of parallel Chrome workers.",
    )

    parser.add_argument(
        "--pause",
        action="store_true",
        help="Pause after each locator page loads. This forces workers to 1.",
    )
    ...
    headed = bool(args.headed or not args.headless)

    workers = max(1, int(args.workers or 1))
    workers = min(workers, len(selected_store_keys))

    if args.pause:
        print("⚠️ --pause enabled, forcing --workers 1.")
        workers = 1

    if args.use_default_profile and workers > 1:
        print("⚠️ Default Chrome profile cannot be opened by multiple Chrome workers.")
        print("⚠️ Forcing --workers 1 because --use-default-profile was selected.")
        workers = 1

    print(f"Running stores: {selected_store_keys}")
    print(f"Workers: {workers}")
    print(f"Chrome visible: {headed}")

    results: dict[str, dict[str, Any]] = {}

    if workers == 1:
        store_key = selected_store_keys[0] if len(selected_store_keys) == 1 else None

        for index, store_key in enumerate(selected_store_keys, start=1):
            key, nearest = run_store_worker(
                worker_id=index,
                store_key=store_key,
                stores_snapshot=stores,
                headed=headed,
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for index, store_key in enumerate(selected_store_keys, start=1):
                ...
                future_map[future] = store_key

            for future in concurrent.futures.as_completed(future_map):
                store_key = future_map[future]

                try:
                    key, nearest = future.result()
                except Exception as exc:
                    key = store_key
                    nearest = {
                        "ok": False,
    ...
    for store_key, nearest in results.items():
        update_store_record(
            stores=stores,
            store_key=store_key,
            nearest=nearest,
