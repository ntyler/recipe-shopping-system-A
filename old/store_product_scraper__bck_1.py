...
def correct_home_store_selected(driver, context: dict[str, Any]) -> bool:
    ...
    required = [address_line_1 or exact_address, pickup_zip]
    optional = [selected_name, store_number]
    ...
    required_ok = all(
        clean_store_text(value).lower() in page_text
        for value in required
        if clean_store_text(value)
    )
    ...
    return bool(required_ok or optional_ok)
