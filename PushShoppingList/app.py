...
<head>
    ...
    <style>
        ...
        .store-manager-row {
            display: grid;
            grid-template-columns: 34px 130px minmax(0, 1fr) 330px;
            gap: 12px;
            align-items: start;
            background: #202425;
            ...
            {% for store_key, store in available_stores.items() %}
                ...
                <div class="store-manager-row">
                    ...
                    <div class="store-manager-url">
                        ...
                        {% if store.password %}
                            <div class="store-detail-line">
                                <span class="store-detail-label">Password</span>
                                Saved ••••••••
                            </div>
                        {% endif %}
                    </div>
                    ...
                    <div class="store-action-row">
                        ...
                        <button type="button"
                                class="store-credentials-btn"
                                onclick="openStoreCredentialsBrowser(
                                    '{{ store.urlStoreSelector or store.url }}'
                                )">
                            Credentials
                        </button>
                ...
                <form id="edit-store-{{ store_key }}"
                    ...
                    <label class="store-edit-field">
                        ...
                        <input name="store_username"
                               ...
                               placeholder="Optional login username or email"
    ...
    <div class="view-control-section">
        ...
        <form method="POST" action="/add_store" onsubmit="saveScroll()">
            ...
            <div class="add-store-help">
                Enter a homepage and the app will guess a search URL. Store Selector URL is the first page opened when setting a store location. Username/password are optional for store automation.
            </div>
                ...
                <div class="item-actions">
                    ...
                    {% if item_has_selected_product(item, item_sources) %}
                        ...
                        <form method="POST" action="/clear_product_pick" class="clear-one-product-form" onsubmit="saveScroll()">
                            <input type="hidden" name="item" value="{{ item }}">
                            <button type="submit" class="clear-one-product-btn">Clear</button>
                        </form>
                    {% else %}
...
function openStoreCredentialsBrowser(storeUrl) {

    if (!storeUrl) {
        alert("This store does not have a credential/login URL saved.");
        return;
    }

    fetch(
        "/open_store_credentials?url=" +
        encodeURIComponent(storeUrl)
    )
        .then(response => response.json())
        .then(data => {
            if (!data.ok) {
                alert(data.error || "Could not open Chrome credentials window.");
            }
        })
        .catch(error => {
            console.error(error);
            alert("Could not open Chrome credentials window. Check the Flask console.");
        });
}
...
def save_all_stores(stores):
    ...
    for raw_key, raw_store in stores.items():
        ...
        username = str(raw_store.get("username") or raw_store.get("store_username") or "").strip()
        password = str(raw_store.get("password") or raw_store.get("store_password") or "").strip()
...
@app.route("/open_store_credentials")
def open_store_credentials():
    ...
    profile_path = (
        r"D:\GitHub\recipe-shopping-system"
        r"\chrome_worker_profiles"
        r"\profile_stores_common_ID"
    )

    # Create the Chrome profile folder if it does not exist
...
def add_store():
    ...
    username = request.form.get("store_username", "").strip()
    password = request.form.get("store_password", "").strip()
