...
def active_cuisine_category_options(user_id=None):
    ...
    try:
        ...
        # The recipe editor must remain usable while a workspace registry is
...
def cookbook_category_choices(user_id=None):
    ...
    choices["cuisine"] = active_cuisine_category_choices(user_id=user_id)
    return choices
...
def purge_cookbook_recipe_urls(cookbook_id, require_unclassified=False, remove_from_other_cookbooks=True):
    with COOKBOOKS_LOCK:
        ...
        if require_unclassified and not is_unclassified_cookbook(target):
