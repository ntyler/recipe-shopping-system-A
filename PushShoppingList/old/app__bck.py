...
def run_bulk_product_job(job_id, enabled_stores=None):
    """Run Grab Best Products through isolated grab_best_products.py."""
    ...
    try:
        if gbp is None:
            def mark_import_error(job):
                job["status"] = "error"
                job["active_item"] = "grab_best_products.py could not be imported"
                job["failed"] = job.get("total", 0)
                job["completed"] = job.get("total", 0)
                job["error"] = "grab_best_products.py could not be imported"

                for entry in job.get("items", []):
                    entry["status"] = "failed"
                    entry["error"] = "grab_best_products.py could not be imported"

            update_bulk_product_job(job_id, mark_import_error)
            clear_active_bulk_job(job_id)
            return
        ...
        # Build the exact URL list BEFORE calling store_product_scraper.py.
        scraper_urls, url_to_item, item_to_url = gbp.build_scraper_urls_for_items(items)
        ...
        # Isolated worker calls store_product_scraper.run_scraper(urls).
        result = gbp.grab_best_products_for_items(items)
        ...
        def mark_finished(job):
            ...
            job["added"] = int(result.get("added", 0) or 0)
            ...
            job["scraper_urls"] = result.get("urls", scraper_urls)
        ...
        if result.get("ok", False):
            send_phone_message(
                f"Best product grab ready for review. Added {result.get('added', 0)}, skipped {result.get('skipped', 0)}, failed {result.get('failed', 0)}."
            )
        else:
...
def start_grab_best_products():
    if gbp is None:
        return redirect("/?message=grab_best_products.py could not be imported")
