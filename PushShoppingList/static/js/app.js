...
let phoneAddressDropPollTimer = null;
...
async function startPhoneAddressDrop(button) {
    const form = document.getElementById("homeAddressForm");
    ...
    if (button) {
        button.disabled = true;
        button.textContent = "Creating drop...";
    }

    try {
        stopPhoneAddressDropPolling();
        const response = await fetch("/api/phone_address_drop/start", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "X-Requested-With": "fetch",
        ...
        if (!response.ok || !data.ok) {
            throw new Error((data && data.error) || "Unable to create phone address drop.");
        }

        renderPhoneAddressDrop(data);
        startPhoneAddressDropPolling(data.status_path);
    } catch (err) {
        console.warn("Unable to start phone address drop.", err);
        setPhoneAddressDropStatus(err.message || "Unable to start phone address drop.", "warning");
    } finally {
        if (button) {
            button.disabled = false;
            button.textContent = originalText || "Phone Address Drop";
        }
    }
}

function renderPhoneAddressDrop(data) {
    const panel = document.getElementById("phoneAddressDropPanel");
    const urlInput = document.getElementById("phoneAddressDropUrl");
    const hint = document.getElementById("phoneAddressDropHint");
    const uploadUrl = new URL(data.upload_path || "", window.location.href).toString();
    ...
    if (hint) {
        hint.textContent = phoneAddressDropOriginHint(uploadUrl);
        hint.classList.toggle("warning", phoneAddressDropUsesLocalhost(uploadUrl));
    }

    setPhoneAddressDropStatus("Waiting for your phone to send an address...", "");
}

function phoneAddressDropOriginHint(uploadUrl) {
    if (phoneAddressDropUsesLocalhost(uploadUrl)) {
        return "This URL uses this device only. On your phone, replace localhost/127.0.0.1 with your computer's LAN or Tailscale address before using it in Shortcuts.";
    }

    return "This is a one-use receiving link. Copy it, then run a phone shortcut that sends current latitude and longitude to the copied URL.";
}

function phoneAddressDropUsesLocalhost(uploadUrl) {
    try {
...
function startPhoneAddressDropPolling(statusPath) {
    if (!statusPath) {
        return;
    }

    const poll = () => pollPhoneAddressDrop(statusPath);
    phoneAddressDropPollTimer = window.setInterval(poll, 2000);
    poll();
}

function stopPhoneAddressDropPolling() {
    if (phoneAddressDropPollTimer) {
        window.clearInterval(phoneAddressDropPollTimer);
        phoneAddressDropPollTimer = null;
    }
}

async function pollPhoneAddressDrop(statusPath) {
    const form = document.getElementById("homeAddressForm");
    ...
    try {
        ...
        if (!response.ok || !data.ok) {
            if (data && data.status === "expired") {
                stopPhoneAddressDropPolling();
                setPhoneAddressDropStatus("Phone address drop expired. Start a new one when you need it.", "warning");
                return;
            }

            throw new Error((data && data.error) || "Unable to check phone address drop.");
        }
        ...
        if (data.status === "received") {
            ...
            try {
                await saveHomeAddressForm(form);
                setPhoneAddressDropStatus("Phone address received and saved.", "success");
                showRecipeQuantityUpdatedMessage("", "", "", "Phone address received and saved.");
            } catch (err) {
                setPhoneAddressDropStatus("Phone address received, but it could not be saved automatically.", "warning");
            }
        ...
        setPhoneAddressDropStatus(`Waiting for your phone... ${formatPhoneAddressDropSeconds(seconds)} remaining.`, "");
    } catch (err) {
        console.warn("Unable to poll phone address drop.", err);
        setPhoneAddressDropStatus(err.message || "Unable to check phone address drop.", "warning");
    }
}

function formatPhoneAddressDropSeconds(seconds) {
...
async function copyPhoneAddressDropUrl(button) {
    const input = document.getElementById("phoneAddressDropUrl");
    ...
    try {
        ...
        input.select();
        setPhoneAddressDropStatus("Copy failed. Select the URL field manually.", "warning");
    }
}

function setPhoneAddressDropStatus(message, tone = "") {
    const panel = document.getElementById("phoneAddressDropPanel");
    const status = document.getElementById("phoneAddressDropStatus");
    ...
    if (status) {
        status.className = `phone-address-drop-status ${tone || ""}`.trim();
        status.textContent = message || "";
    }
}
