...
phone_address_drops = {}
PHONE_ADDRESS_DROP_SECONDS = 10 * 60
...
@main_bp.route("/api/phone_address_drop/start", methods=["POST"])
def start_phone_address_drop_route():
    cleanup_phone_address_drops()
    data = request.get_json(silent=True) or {}
    token = secrets.token_urlsafe(18)
    expires_at = time.time() + PHONE_ADDRESS_DROP_SECONDS

    phone_address_drops[token] = {
        "address": None,
        "created_at": time.time(),
        "current_address": normalize_address_form_fields(
            data.get("current_address") if isinstance(data, dict) else {}
        ),
        "display_name": "",
    ...
    return jsonify({
        "ok": True,
        "token": token,
        "expires_in": PHONE_ADDRESS_DROP_SECONDS,
        "upload_path": f"/api/phone_address_drop/{token}",
        "status_path": f"/api/phone_address_drop/{token}/status",
    })


@main_bp.route("/api/phone_address_drop/<token>", methods=["GET", "POST"])
def receive_phone_address_drop_route(token):
    drop, error_response = get_phone_address_drop_or_error(token)

    if error_response:
        if request.method == "GET":
            return phone_address_drop_landing_html(
                "This phone address drop expired or does not exist.",
                expired=True,
            ), 410
    ...
    if request.method == "GET":
        return phone_address_drop_landing_html(
            "Ready to receive your current location or full address.",
            expired=False,
        )

    if drop.get("address"):
        if phone_address_drop_wants_html_response():
            return phone_address_drop_landing_html(
                "This phone address drop already received an address. Return to the shopping page.",
                expired=True,
            ), 409

        return jsonify({
            "ok": False,
            "error": "This phone address drop has already received an address.",
        }), 409
    ...
    try:
        address, display_name = normalize_phone_address_drop_payload(
            data,
            drop.get("current_address") or {},
        )
    except ValueError as exc:
        if phone_address_drop_wants_html_response():
            return phone_address_drop_landing_html(str(exc), expired=False), 400
        ...
        if phone_address_drop_wants_html_response():
            return phone_address_drop_landing_html(
                f"Unable to receive phone address: {exc}",
                expired=False,
            ), 502

        return jsonify({
            "ok": False,
            "error": f"Unable to receive phone address: {exc}",
        }), 502

    drop["address"] = address
    drop["display_name"] = display_name
    drop["received_at"] = time.time()

    if phone_address_drop_wants_html_response():
        return phone_address_drop_landing_html(
            "Address received. Return to the shopping page.",
            expired=True,
            success=True,
        )
...
@main_bp.route("/api/phone_address_drop/<token>/status", methods=["GET"])
def phone_address_drop_status_route(token):
    drop, error_response = get_phone_address_drop_or_error(token)
    ...
    return jsonify({
        "ok": True,
        "status": "waiting",
        "expires_in": max(0, int(drop["expires_at"] - time.time())),
    })
...
def cleanup_phone_address_drops():
    now = time.time()
    expired_tokens = [
        token
        for token, drop in phone_address_drops.items()
        if drop.get("expires_at", 0) <= now
    ]

    for token in expired_tokens:
        phone_address_drops.pop(token, None)


def get_phone_address_drop_or_error(token):
    cleanup_phone_address_drops()
    drop = phone_address_drops.get(token)

    if not drop:
        return None, (
            jsonify({
                "ok": False,
                "error": "This phone address drop expired or does not exist.",
                "status": "expired",
            }),
            410,
        )
...
def phone_address_drop_wants_html_response():
    return (
        not request.is_json
...
def phone_address_drop_landing_html(message, expired=False, success=False):
    status_class = "success" if success else "warning" if expired else ""
    disable_controls = "disabled" if expired or success else ""
...
<head>
    ...
    <title>Phone Address Drop</title>
    <style>
        body {{
            margin: 0;
            padding: 18px;
...
<body>
    <main class="card">
        <h1>Phone Address Drop</h1>
        <p>This one-use page sends your phone location back to the shopping list page that created it.</p>
        <div id="status" class="status {status_class}">{safe_message}</div>
        <button type="button" onclick="sendCurrentLocation()" {disable_controls}>Send Current Location</button>
        <form method="post">
            <label for="full_address">Or paste a full address</label>
            <textarea id="full_address" name="full_address" placeholder="5905 Arlo Drive Apt 2213, Indianapolis, IN 46237" {disable_controls}></textarea>
            <button class="manual-btn" type="submit" {disable_controls}>Send Full Address</button>
        </form>
        <p>If <strong>Send Current Location</strong> says the page is not secure, use the iPhone Shortcut POST flow instead. Browser GPS still requires HTTPS.</p>
        <p>Shortcut body can be <code>{{"latitude": 39.0, "longitude": -86.0}}</code>.</p>
    </main>
    <script>
        ...
        async function sendCurrentLocation() {{
            if (!window.isSecureContext) {{
                setStatus("Browser GPS is blocked because this page is Not Secure. Use the iPhone Shortcut POST flow, or open the app through trusted HTTPS.", "warning");
                return;
            }}

            if (!navigator.geolocation) {{
                setStatus("This browser cannot use device location.", "warning");
                return;
            }}
            ...
            navigator.geolocation.getCurrentPosition(async position => {{
                try {{
                    setStatus("Sending location to shopping page...", "");
                    ...
                    if (!response.ok || !data.ok) {{
                        throw new Error((data && data.error) || "Unable to send location.");
                    }}

                    setStatus("Location sent. Return to the shopping page.", "success");
                }} catch (err) {{
                    setStatus(err.message || "Unable to send location.", "warning");
                }}
            }}, err => {{
                setStatus(err.message || "Unable to get current location.", "warning");
            }}, {{
...
def normalize_phone_address_drop_payload(data, current_address):
    if not isinstance(data, dict):
        raise ValueError("Send the address as JSON or form fields.")

    latitude = parse_phone_address_coordinate(data, "latitude", "lat")
    longitude = parse_phone_address_coordinate(data, "longitude", "lng", "lon")
    nested_address = data.get("address") if isinstance(data.get("address"), dict) else {}
    candidate_address = normalize_address_form_fields({
        **data,
        **nested_address,
    })
    display_name = first_phone_address_text(data, [
        "display_name",
        "formatted_address",
        "full_address",
        "address_text",
    ...
    if not display_name and isinstance(data.get("address"), str):
        display_name = str(data.get("address") or "").strip()

    if latitude is not None and longitude is not None and address_needs_completion(candidate_address):
        reverse_result = reverse_geocode_coordinates(latitude, longitude)
        display_name = display_name or reverse_result.get("display_name", "")
        candidate_address = complete_address_fields_locally(
            reverse_result.get("address") or {},
            candidate_address,
            display_name,
        )
    ...
    completed_address = complete_address_fields_locally(
        candidate_address,
        normalize_address_form_fields(current_address),
        display_name,
    )
    ...
    if not address_has_any_value(completed_address):
        raise ValueError("No address fields were found in the phone payload.")

    return completed_address, display_name


def parse_phone_address_coordinate(data, *keys):
    for key in keys:
        value = data.get(key)
...
def first_phone_address_text(data, keys):
    for key in keys:
...
def address_has_any_value(address):
    return any(
