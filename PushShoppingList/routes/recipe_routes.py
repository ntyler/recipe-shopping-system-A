...
from PushShoppingList.services.ingredient_type_service import ingredient_type_registry_payload
...
from PushShoppingList.services.cookbook_item_inference_service import regenerate_recipe_description_for_recipe
...
def edit_recipe_page_route():
    ...
    return render_template(
        ...
        ingredient_type_config=ingredient_type_registry_payload(),
...
@recipe_bp.route("/api/recipe/regenerate_description", methods=["POST"])
def api_recipe_regenerate_description_route():
    data = request.get_json(silent=True) or {}
    recipe_url = str(
        data.get("url")
        or data.get("original_url")
    ...
    result = regenerate_recipe_description_for_recipe(
        recipe_url,
        current_recipe=data.get("recipe") if isinstance(data.get("recipe"), dict) else None,
        cookbook_id=str(data.get("cookbook_id") or "").strip(),
        cookbook_name=str(data.get("cookbook_name") or "").strip(),
