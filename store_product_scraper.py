...
def load_default_stores():
    path = Path(SHOPPING_STORES_FILE)
    ...
    for index, (store_key, store_data) in enumerate(stores.items(), start=1):
        parsed = urlparse(store_data["url"])

        ordered[store_key] = {
            "label": store_data.get("label", store_key.title()),
            "url": store_data["url"],
            "base_url": f"{parsed.scheme}://{parsed.netloc}",
            "urlStoreSelector": store_data.get("urlStoreSelector", ""),
            "username": store_data.get("username", ""),
            "password": store_data.get("password", ""),
            "location": store_data.get("label", store_key.title()),
            "order": index,
        }
...
DEFAULT_STORES = load_default_stores()


# =========================================================
# PROGRESS / POPUP STATUS SUPPORT
# =========================================================
# app.py can pass a progress_callback into run_scraper().
# The scraper owns the progress messages; Flask/browser only displays them.
...
def emit_progress(
    ...
    *,
    ...
    try:
        ...
        # Backward-compatible callback shape: callback(current, total, message)
        progress_callback(completed, total, message)  # type: ignore[misc]
    except Exception as exc:
        print(f"⚠️ Progress callback failed: {exc}")


def make_progress_file_callback(progress_file: str | Path | None) -> ProgressCallback | None:
...
def merge_store_config(
    ...
    # urlStoreSelector is the first store-specific page opened when
    # setting/changing the home store for each worker.
    base_url = str(
        raw_store.get("urlStoreSelector")
        or raw_store.get("base_url")
        or ""
    ).strip()

    if not base_url:
        website = str(
            result_store.get("urlStoreSelector")
            or result_store.get("website")
            or ""
        ).strip()
    ...
    return {
        ...
        "base_url": base_url,
        "urlStoreSelector": raw_store.get("urlStoreSelector") or result_store.get("urlStoreSelector"),
        "username": raw_store.get("username") or result_store.get("username") or "",
        "password": raw_store.get("password") or result_store.get("password") or "",
        "location": location,
        ...
        "website": (
            raw_store.get("urlStoreSelector")
            or result_store.get("urlStoreSelector")
            or result_store.get("website")
        ),
...
def init_driver(
    ...
    # Persistent per-worker Chrome profile.
    # Run inject_chrome_passwords.py BEFORE the scraper to seed this profile
    # with passwords from PushShoppingList/shopping_stores.json.
    profile_dir = SCRIPT_DIR / "chrome_worker_profiles" / f"worker_{worker_id}"
    profile_dir.mkdir(parents=True, exist_ok=True)

    print(f"[Worker {worker_id}] Chrome profile: {profile_dir}")
...
def worker_loop(
    ...
    try:
        ...
        while True:
            ...
            mode = job.get("mode")
            active_item = job.get("item") or search_query_from_url(job.get("url", ""))
            active_url = job.get("url")
            ...
            emit_progress(
                ...
                total=total_jobs,
                message=f"Worker {worker_id} reviewing {active_item}",
                active_item=active_item,
                active_url=active_url,
                worker=worker_id,
            )
            ...
            print(f"{status_icon} [Worker {worker_id}] finished")

            result_label = result.get("query") or result.get("url") or active_item
            emit_progress(
                progress_callback,
                status="running",
                completed=current_completed,
                total=total_jobs,
                message=f"Finished {current_completed}/{total_jobs}: {result_label}",
                active_item=str(result_label or ""),
                active_url=result.get("url") or active_url,
                worker=worker_id,
                result=result,
            )
...
def run_worker_queue(
    ...
    emit_progress(
        ...
        completed=0,
        total=len(jobs),
        message=f"Queued {len(jobs)} product searches with {workers} workers",
    )
    ...
    emit_progress(
        ...
        total=len(jobs),
        message=f"Finished {len(results)}/{len(jobs)} product searches",
    )
...
def build_home_store_context(store: str) -> dict[str, Any]:
    ...
    # Pull the user's home address from shopping_stores_Results.json too.
    # This matters for Aldi: its store picker wants the HOME address/ZIP in the
    # "Near ..." field, then we choose the saved nearest store card by address.
    home_address: dict[str, Any] = {}
    try:
        raw_results = read_json_file(find_store_results_file(), {})
        home_address, _store_results = normalize_store_results_payload(raw_results)
    except Exception:
        home_address = {}

    home_street = clean_store_text(home_address.get("street"))
    home_apartment = clean_store_text(home_address.get("apartment"))
    home_city = clean_store_text(home_address.get("city"))
    home_state = clean_store_text(home_address.get("state"))
    home_zip = clean_store_text(home_address.get("zip"))
    home_full_address = clean_store_text(home_address.get("full_address"))

    if not home_full_address:
        street_line = " ".join(part for part in [home_street, home_apartment] if part).strip()
        city_state = ", ".join(part for part in [home_city, home_state] if part).strip()
        if city_state and home_zip:
            city_state = f"{city_state} {home_zip}"
        home_full_address = ", ".join(part for part in [street_line, city_state] if part).strip()
    ...
    website = clean_store_text(
        store_info.get("urlStoreSelector")
        or store_info.get("website")
    )
    base_url = clean_store_text(
        store_info.get("urlStoreSelector")
        or store_info.get("base_url")
    )
    pickup_zip = (
    ...
    # Search the store locator with HOME address first, not the store address.
    search_values = []
    for value in [
        home_full_address,
        " ".join(part for part in [home_street, home_zip] if part).strip(),
        home_zip,
        pickup_zip,
        exact_address,
        selected_name,
    ...
    return {
        ...
        "exact_address": exact_address,
        "pickup_zip": pickup_zip,
        "urlStoreSelector": clean_store_text(store_info.get("urlStoreSelector")),
        "website": website,
        "base_url": base_url,
        "store_number": store_number,
        "home_address": home_full_address,
        "home_street": home_street,
        "home_zip": home_zip,
        "username": clean_store_text(store_info.get("username")),
        "password": clean_store_text(store_info.get("password")),
...
def type_visible_location_input(driver, values: list[str], wait: float = 1.5) -> bool:
    ...
    # Aldi uses a "Near ..." address field. Other stores use ZIP/address/search fields.
    input_xpaths = [
            ...
            for box in boxes:
                try:
                    if box.is_displayed() and box.is_enabled():
                        ...
                        # clear() is not enough on some React inputs.
                        try:
...
def click_aldi_change_store_flow(
    driver,
    ...
    """
    Aldi-specific flow:
      1. Click Change store / Find Stores.
      2. Enter the user's HOME address/ZIP in the Near field.
      3. Click the saved nearest Aldi store card by exact store address/name.
      4. Confirm the selected Aldi store is visible.
    """
    ...
    print(f"[Worker {worker_id}] 🛒 Aldi flow: opening Change store")

    # If the pickup popover is already open, Change store is visible. If not,
    # click the pickup/find-stores header first, then click Change store.
    clicked_location = click_text_button(
        driver,
        [
            "Change store",
            "Find Stores",
    ...
    # Sometimes clicking the pickup chip only opens the dropdown; then the real
    # Change store link appears inside it.
    clicked_change_inside_dropdown = click_text_button(
        driver,
        ["Change store", "Choose an In-Store location", "Choose an In-Store Location"],
        wait=wait_seconds,
    )

    clicked_location = bool(clicked_location or clicked_change_inside_dropdown)
    accept_cookies_if_present(driver, wait=0.75)

    print(f"[Worker {worker_id}] 🛒 Aldi flow: entering home address/ZIP")
    ...
    # Aldi can show results after typing, but sometimes needs one extra Enter/click.
    accept_cookies_if_present(driver, wait=0.75)
    time.sleep(1)

    print(f"[Worker {worker_id}] 🛒 Aldi flow: selecting saved store card")
    ...
    # Some Aldi pages select immediately; some show a continue/save/start button.
    ...
    message = "Aldi home store update attempted."
    if confirmed:
        message = "Aldi home store confirmed."
    else:
        message = "Aldi home store was not confirmed. Retrying if attempts remain."

    print(f"[Worker {worker_id}] {'✅' if confirmed else '⚠️'} aldi: {message}")
...
def correct_home_store_selected(driver, context: dict[str, Any]) -> bool:
    ...
    # Best confirmation: the exact store address or first address line is visible.
    if contains(exact_address) or contains(address_line_1):
        return True

    # Aldi often confirms with the store label in the top pickup chip, e.g.
    # "ALDI - GRE 73 - Indianapolis", while the physical address may only be
    # visible during the selection list.
    if contains(selected_name):
        return True

    if contains(store_number):
        return True

    # ZIP alone is weak, but acceptable only if paired with some store/name hint.
    if contains(pickup_zip) and (contains(selected_name) or contains(store_number)):
        return True
...
def find_store_script_path(store: str) -> Path | None:
    store = clean_store_text(store).lower()
    candidates = [
        SCRIPT_DIR / "store_scripts" / f"{store}_store.py",
        SCRIPT_DIR / "PushShoppingList" / "store_scripts" / f"{store}_store.py",
    ]
...
def load_store_script(store: str):
    script_path = find_store_script_path(store)

    if not script_path:
        return None

    module_name = f"store_scripts_{clean_store_text(store).lower()}_store"
    ...
    try:
        ...
        print(f"⚠️ Could not load store script {script_path}: {exc}")
        return None
...
def update_home_store_on_existing_driver(
    ...
    store_script = load_store_script(store)

    if store_script and hasattr(store_script, "update_home_store"):
        print(f"[Worker {worker_id}] 🧩 Using store script for {store}")
        try:
            return store_script.update_home_store(
                driver=driver,
                context=context,
            ...
            print(f"[Worker {worker_id}] ⚠️ Store script failed for {store}; falling back to generic flow: {exc}")
    ...
    if not start_url:
        return {
            ...
            "ok": False,
            "store_key": store,
            "message": "No store selector/website/base URL found.",
            **context,
        }

    print(f"[Worker {worker_id}] 🏬 First time seeing {store}; using generic home-store flow")
    print(f"[Worker {worker_id}] 🌐 Opening generic store selector: {start_url}")
...
##        ok = bool(confirmed or clicked_continue or clicked_store_card or clicked_final or already_visible)
        ...
        if not ok:
            print(f"[Worker {worker_id}] ⚠️ {store}: correct address was NOT confirmed")
...
def ensure_home_store_for_worker(
    ...
    if store in worker_home_stores_done:
        previous = worker_home_store_updates.get(store)

        if previous and previous.get("ok"):
            return previous

        # If previous attempt was bad, do NOT trust it.
        worker_home_stores_done.discard(store)

    last_result = None

    for attempt in range(1, max_attempts + 1):
        print(f"[Worker {worker_id}] 🏬 Setting home store for {store}, attempt {attempt}/{max_attempts}")
        ...
        if result.get("ok") and confirmed:
            print(f"[Worker {worker_id}] ✅ Correct address confirmed for {store}")
            worker_home_stores_done.add(store)
            return result

        print(f"[Worker {worker_id}] ⚠️ Correct address not selected for {store}; retrying...")
    ...
    print(f"[Worker {worker_id}] ❌ Could not confirm correct address for {store} after {max_attempts} attempts")

    if last_result:
        last_result["ok"] = False
        last_result["message"] = f"Could not confirm correct address after {max_attempts} attempts."

    worker_home_store_updates[store] = last_result or {
        "attempted": True,
        "ok": False,
        "store_key": store,
        "message": f"Could not confirm correct address after {max_attempts} attempts.",
    }

    return worker_home_store_updates[store]
...
def run_scraper(
    ...
    app.py should call this with a list of store search/product URLs.
    The popup/progress wording now lives here through progress_callback.
    """
    urls = [
        str(url).strip()
    ...
    if not urls:
        emit_progress(
            ...
            completed=0,
            total=0,
            message="No product URLs to review.",
        )
        return {
            "ok": False,
            "mode": "url_file_worker_queue",
            "total": 0,
            "workers": 0,
            "results": [],
            "error": "No product URLs to review.",
        }
...
def main() -> int:
    ...
    parser.add_argument(
        "--progress-file",
        help="Optional JSON file updated while scraping. app.py can poll this for popup status.",
    )
    ...
    progress_callback = make_progress_file_callback(args.progress_file)
...
if __name__ == "__main__":
    # Normal CLI flags use argparse/main().
    # Plain positional URLs still work for app.py/subprocess compatibility:
    #   py -3.11 store_product_scraper.py https://site/search?q=carrots https://site/search?q=onion
    if len(sys.argv) > 1 and not sys.argv[1].startswith("-"):
        cli_urls = [arg for arg in sys.argv[1:] if str(arg).startswith("http")]
        result = run_scraper(cli_urls)
        print(json.dumps(result, indent=2, ensure_ascii=False))
